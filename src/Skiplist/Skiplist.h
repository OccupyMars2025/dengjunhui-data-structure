...
struct Skiplist : public Dictionary<K, V>, public List<Quadlist<Entry<K, V>>*> {
   Skiplist() { List<Quadlist<Entry<K, V>>*>::insertAsFirst( new Quadlist< Entry<K, V> > ); }; //即便为空，也有一层空列表
   QNodePosi< Entry<K, V> > search( K ); //由关键码查询词条
   Rank size() const { return List<Quadlist<Entry<K, V>>*>::empty() ? 0 : List<Quadlist<Entry<K, V>>*>::last()->data->_size; } //词条总数
