...
bool Skiplist<K, V>::put( K k, V v ) { //跳转表词条插入算法
   ...
   ListNodePosi< Quadlist< Entry<K, V> >* > qlist = List<Quadlist<Entry<K, V>>*>::last(); //首先在最底层
   ...
   while ( rand() & 1 ) { //经投掷硬币，若确定新塔需要再长高，则
      ...
      if ( !p->pred && !p->above ) { //若该前驱是header，且已是最顶层，则
         List<Quadlist<Entry<K, V>>*>::insertAsFirst( new Quadlist< Entry<K, V> > ); //需要创建新的一层
         List<Quadlist<Entry<K, V>>*>::first()->data->header->below = qlist->data->header;
         qlist->data->header->above = List<Quadlist<Entry<K, V>>*>::first()->data->header;
      }
