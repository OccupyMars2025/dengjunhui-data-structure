...
such that >=, <=, !=, == and < define a total order on T
*/
...
*/
template <typename T>
int interpolation_search_v001(T* arr, const T& key, int low, int high)
{
    high -= 1;
    int middle;
    int initial_low = low;
...
*/
template <typename T> 
int interpolation_search_v002(T* arr, const T& key, int low, int high) {
    high -= 1;
    assert(low <= high);
    int middle;
